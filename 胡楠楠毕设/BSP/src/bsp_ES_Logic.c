...
static double ES_Table_Val[4] = 0 ;//获取的电压值
 
double* Get_ES_Table_Val(void)	//外部调用获取电压值
{
	return(ES_Table_Val);
}


static double Get_ES_Val(void)	//内部调用获取电压值
{
	return(*ES_Val);
}

static double Get_ES_Val2(void)	//内部调用获取电压值
{
	return(*(ES_Val + 1));
}
