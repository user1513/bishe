...
#line 1 "source\matrix_keyboard.c" /0
...
#line 15 "SOURCE\MAIN.H" /0
 ...
 int8_t matrix_keyboard_scan(void);
...
#line 149 "SOURCE\MAIN.H" /1
 
 
 
 
#line 153 "SOURCE\MAIN.H" /0
...
#line 13 "source\matrix_keyboard.c" /0
 ...
 static uint8_t code matrix_keyboard_code_tab[]={
 ...
 static void matrix_keyboard_delay_5ms(void)
 {
 unsigned char i, j;
 ...
 int8_t matrix_keyboard_scan(void)
 {
 uint8_t sCode,kCode,i,k;
 
  P1 = 0xf0;
 if((P1 & 0xf0) != 0xf0){
 matrix_keyboard_delay_5ms();
 if((P1 & 0xf0) != 0xf0){
 sCode = 0xfe;
 for(k = 0; k < 4; k++){
  P1 = sCode;
 ...
 kCode = ~P1;
 for(i = 0; i < 16; i++){
 if(kCode == matrix_keyboard_code_tab[i]){
 for(k = 0; k < 50; k++){
 matrix_keyboard_delay_5ms();
 kCode = ~P1;
 if(kCode != matrix_keyboard_code_tab[i]){
